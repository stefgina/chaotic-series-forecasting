...
Date: 2023-05-15
Description: EDA, Visualization, Stat Analysis, TCNN Model - Chaotic Series Forecasting
"""
...
class Parameters():
    ...
    def __init__(self):
        ...
        # trained model path
        self.model_path    = 'TCN32x32_b512_l20_PROPER.pth'
        # patience
        self.patience      = 100
...
if args.corelation:
    """
    Calculate and visualize the correlation matrices between datasets.

    - Calculates correlation matrices using np.corrcoef for different combinations of train_data, val_data, and test_data.
    - Plots the correlation matrices as heatmaps using plt.imshow.
    - Adds colorbars to the heatmaps using fig.colorbar.
    - Displays the plot using plt.show.
    """
    correlation_matrix1 = np.corrcoef(train_data.squeeze(), test_data.squeeze())
    correlation_matrix2 = np.corrcoef(train_data.squeeze(), val_data.squeeze())
    correlation_matrix3 = np.corrcoef(val_data.squeeze(), test_data.squeeze())
    ...
    # plot the individual correlation matrices as heatmaps
    axes[0].imshow(correlation_matrix1, cmap='coolwarm', vmin=-1, vmax=1)
    axes[0].set_title('TRAIN-TEST')

    axes[1].imshow(correlation_matrix2, cmap='coolwarm', vmin=-1, vmax=1)
    axes[1].set_title('TRAIN-VAL')

    axes[2].imshow(correlation_matrix3, cmap='coolwarm', vmin=-1, vmax=1)
    axes[2].set_title('VAL-TEST')

    # add colorbars
    cbar1 = fig.colorbar(axes[0].imshow(correlation_matrix1, cmap='coolwarm', vmin=-1, vmax=1), ax=axes[0])
    cbar2 = fig.colorbar(axes[1].imshow(correlation_matrix2, cmap='coolwarm', vmin=-1, vmax=1), ax=axes[1])
    cbar3 = fig.colorbar(axes[2].imshow(correlation_matrix3, cmap='coolwarm', vmin=-1, vmax=1), ax=axes[2])
...
######################  pearson datasets correlation
if args.corelation:
    """
    Calculate and visualize the Pearson correlation coefficients and p-values between datasets.

    - Calculates the Pearson correlation coefficient and p-value between train_data and val_data using pearsonr.
    - Prints the correlation coefficient and p-value.
    - Computes the pairwise Pearson correlation coefficients and p-values between train_data and test_data.
    - Plots the p-value matrix and correlation matrix using plt.imshow.
    """
    # calculate the Pearson correlation coefficient and p-value
    corr_coeff, p_value = pearsonr(train_data.flatten(), val_data.flatten())
    ...
    p_value_matrix = np.zeros((100, 100))

    # compute the pairwise Pearson correlation coefficients and p-values
    for i in range(100):
        for j in range(100):
            corr_coeff, p_value = pearsonr(train_data.squeeze()[i], test_data.squeeze()[j])
            correlation_matrix[i, j] = corr_coeff
            p_value_matrix[i, j] = p_value


    # plot the array
    plt.imshow(p_value_matrix, cmap='viridis')
    plt.colorbar()
...
class TemporalCNNForecast(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        super(TemporalCNNForecast, self).__init__()
    ...
    def forward(self, input_seq):
        input_seq = input_seq.permute(0, 2, 1)  # Reshape input for Conv1d (batch_size, input_size, sequence_length)
        conv1_out = F.relu(self.conv1(input_seq))
        conv2_out = F.relu(self.conv2(conv1_out))
        conv2_out = conv2_out.permute(0, 2, 1)  # Reshape output for Linear layer (batch_size, sequence_length, hidden_size)
        predictions = self.fc(conv2_out[:, -1, :])  # take the last output of each sequence in the batch
        return predictions
...
model = TemporalCNNForecast(input_size, hidden_size, output_size)
